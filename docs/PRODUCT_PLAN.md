...
4. **앱 데이터가 원본:** Google Sheets 연동을 도입하더라도 앱에서 Sheets로 보내는 단방향 동기화부터 시작한다.
...
- 할인·쿠폰 전용 데이터 모델은 두지 않으며 사용자는 최종 결제 금액을 거래와 품목에 입력한다.
...
Google Sheets 연동은 P0와 최초 계정 전환 범위에서 제외한다. 필수 계정과 클라우드 동기화가 안정된 뒤 P2에서 다시 검토한다.
...
| `transactions` | 결제 또는 수입 | `type`, `name`, `total_amount`, `occurred_at`, `category_id`, `merchant_id`, `status`, `recurring_rule_id`, `scheduled_date` |
...
| `recurring_rules` | 반복 규칙 | `type`, `name`, `total_amount`, `category_id`, `frequency`, `interval`, `start_date`, `end_date`, `next_scheduled_date`, `requires_confirmation`, `is_active` |
...
- 로컬 스키마는 버전형 전진 마이그레이션으로 관리한다. v1은 `transactions`와 `transaction_items`, v2는 `categories`와 기본 카테고리 시드, v3는 자동완성 기준 데이터인 `merchants`와 `products`, v4는 iOS 거래·품목 표시명 스냅샷, v5는 반복 규칙과 반복 거래 상태·예정일이다.
...
- 반복 거래는 앱 시작 및 규칙 저장 직후 기기 현지 날짜 기준으로 오늘까지 밀린 일정을 생성한다. 같은 규칙과 예정일은 `(recurring_rule_id, scheduled_date)` 고유 제약으로 한 번만 생성하고, 삭제한 예정 거래는 규칙의 다음 예정일을 되돌리지 않아 사용자의 제외 의사를 유지한다.
- P0 반복 거래의 기본 생성 시각은 현지 시각 오전 9시다. 자동 확정 규칙은 즉시 확정 거래를 만들고, 금액 확인 필요 규칙은 사용자가 거래 수정 화면에서 저장할 때 확정한다. P0에서는 로컬 알림 권한을 요청하지 않는다.
- 월별 수입·지출·잔액과 카테고리별 지출은 `confirmed` 거래만 집계한다. `needs_confirmation` 반복 거래는 사용자가 확인·저장해 `confirmed`가 된 뒤부터 통계에 포함한다.
...
- P0 로컬 모드는 로그인 없이 사용할 수 있다.
- 계정과 클라우드 동기화를 제품에 연결하는 시점부터 신규 사용은 계정 가입을 필수로 전환한다. 기존 로컬 데이터의 계정 귀속·이관 절차를 먼저 설계한다.
...
- [x] iPhone과 Web에서 실행된다.
- [x] 인터넷 없이 거래를 등록할 수 있다.
- [x] 앱을 재실행해도 로컬 거래가 유지된다.
- [x] 한 거래에 여러 상세 품목을 등록할 수 있다.
- [x] 거래와 품목을 조회, 수정, 삭제할 수 있다.
- [x] 거래명, 상점명, 제품명 자동완성이 작동한다.
- [x] 제품 검색 결과에 과거 구매 날짜와 가격이 표시된다.
- [x] 반복 거래가 동일 예정일에 중복 생성되지 않는다.
- [x] 월별 수입, 지출, 잔액이 정확하다.
- [x] 상세 품목이 있어도 통계 금액이 이중 집계되지 않는다.
- [x] 작은 iPhone 화면과 데스크톱 Web에서 주요 UI가 깨지지 않는다.
...
- [x] 할인·쿠폰을 음수 품목과 조정 필드 중 어느 방식으로 관리할지 — 별도 할인·쿠폰 모델을 범위에서 제거하고 거래·품목에는 최종 결제 금액만 입력한다. (2026-08-30)
- [x] 제품명을 사용자가 직접 병합할 수 있게 할지 — P0 이후 사용자가 필요할 때 선택적으로 실행하는 수동 병합 기능으로 남긴다. 자동 병합은 하지 않는다. (2026-08-30)
- [x] 반복 거래 생성 시점과 알림 정책 — 앱 시작 및 규칙 저장 직후 오늘까지의 일정을 생성하고, 고유 예정일 제약으로 재실행 중복을 막는다. 기본 생성 시각은 현지 오전 9시이며 P0 알림은 비활성화한다. 금액 확인 필요 거래는 수정·저장 시 확정한다. (2026-08-29)
...
- [x] P0 반복 거래와 사용자 카테고리 로컬 스키마 — v5에서 반복 규칙과 거래의 `status`, `recurring_rule_id`, `scheduled_date`를 추가한다. 사용자 카테고리는 유형별 마지막 활성 항목을 제외하고 숨김·재활성화할 수 있으며 기존 거래의 카테고리 연결은 유지한다. (2026-08-29)
- [x] `금액 확인 필요` 반복 거래를 Day 6 월별·카테고리 통계에 포함할지, 확정 전까지 제외할지 — `needs_confirmation` 상태에서는 월별·카테고리 통계에서 제외하고, 사용자가 거래를 확인·저장해 `confirmed`가 된 뒤 포함한다. 화면에는 제외된 거래 건수를 안내한다. (2026-08-30)
- [x] P0 이후 계정·배포 전환 순서 — P0는 계정 가입 없이 로컬 모드로 완료한다. 계정·클라우드를 연결할 때 가입을 필수로 전환하고, Web 실제 배포도 그 시점에 진행한다. 전환 전 Web은 로컬 정적 프리뷰, iOS는 로컬 개발 빌드로 검증한다. (2026-08-30)
- [x] Google Sheets 월별 탭의 실제 레이아웃 — 계정·클라우드 동기화가 안정된 이후 P2에서 레이아웃과 권한 범위를 다시 결정한다. 현재 구현에서는 제외한다. (2026-08-30)

## 18. Day 7 검증 결과와 알려진 문제

### 검증 결과

- Web은 정적 export를 로컬 프리뷰 서버에서 제공하고 `/`, `/stats` 등 정적 라우트 응답을 확인한다.
- iOS는 Expo 계정 없이 `expo run:ios`로 Simulator용 네이티브 개발 빌드를 생성한다.
- Web은 앱을 먼저 연 상태에서 인터넷 연결을 끊어도 IndexedDB에 거래를 저장·조회할 수 있고, 다시 실행한 페이지에서도 유지된다.
- iOS는 앱 종료 상태에서도 SQLite 거래가 유지되고 로컬 개발 빌드를 재실행하면 월 통계에 다시 반영된다.
- 빈 데이터, IndexedDB 오류, 안전 정수 범위의 큰 금액, 여러 달 이동, 기본 접근성 이름과 라이브 오류 안내를 자동 검증한다.

### 알려진 문제

- 실제 Web 배포를 연기했으므로 현재 Web 프리뷰는 로컬 서버가 실행 중이어야 처음 열 수 있다. 이미 열린 앱의 로컬 CRUD는 인터넷 없이 동작하지만, 서버까지 없는 완전한 오프라인 콜드 스타트는 지원하지 않는다.
- iPhone SQLite와 Web IndexedDB는 아직 서로 동기화되지 않는다.
- 자동 접근성 검증은 레이블·역할·오류 알림 중심이며 실제 VoiceOver 수동 사용성 점검은 남아 있다.
- Day 7 이전 개발 환경에서 자동 생성된 `Day 1 로컬 저장 테스트` 거래는 데이터 보존을 위해 자동 삭제하지 않는다. 필요하면 개발 데이터에서 사용자가 직접 삭제한다.
- 홈의 실제 최근 거래·반복 예정 목록과 상점·품목 통계는 P0 이후 작업이다.

## 19. 다음 버전 작업 목록

1. 필수 계정 가입과 기존 로컬 데이터의 안전한 계정 귀속·이관
2. Supabase 기반 백업·기기 간 동기화와 RLS 정책 테스트
3. 계정 전환 후 Web 실제 배포와 오프라인 콜드 스타트 전략 검토
4. 사용자가 선택적으로 실행하는 제품명 수동 병합
5. 홈의 최근 거래·반복 예정 목록과 상점·품목별 통계
6. VoiceOver 실기기 점검과 포커스 순서·동적 글자 크기 보완
7. 계정·클라우드 안정화 이후 Google Sheets 연동 재검토
